...
public class SudokuController {
     ...
     * {@code index}, if given, picks that exact bank entry for a brand-new attempt - see
     * {@link SudokuService#getPuzzleForUser(CallerIdentity, Integer)}.
     */
    @GetMapping("puzzle")
    public PuzzleResponse getPuzzle(@RequestParam(required = false) Integer index, HttpServletRequest request) {
        return sudokuService.getPuzzleForUser(currentIdentity(request), index);
    }
