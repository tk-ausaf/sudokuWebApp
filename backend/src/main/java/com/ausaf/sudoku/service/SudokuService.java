...
public class SudokuService {
    ...
    public PuzzleResponse getPuzzleForUser(CallerIdentity identity) {
        return getPuzzleForUser(identity, null);
    }

    /**
     * Same as {@link #getPuzzleForUser(CallerIdentity)}, but when a brand new attempt actually
     * needs to be created, {@code preferredBankIndex} (if non-null and in range) picks that exact
     * bank entry instead of a random one - so a frontend that already rendered that same bank
     * index from its own bundled copy of the puzzle file can swap in this real, backend-tracked
     * attempt with no visible change to the displayed grid. Ignored when resuming an existing
     * in-progress attempt, since one already exists.
     */
    public PuzzleResponse getPuzzleForUser(CallerIdentity identity, Integer preferredBankIndex) {
        ...
        Optional<GeneratedMultiplayerPuzzle> preferred = preferredBankIndex == null
                ? Optional.empty() : puzzleBankService.getPuzzleAt(preferredBankIndex);
        String clueGrid = preferred.or(puzzleBankService::getRandomPuzzle).map(GeneratedMultiplayerPuzzle::clueGrid)
                .orElseGet(() -> {
