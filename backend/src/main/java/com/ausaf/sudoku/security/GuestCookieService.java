...
public class GuestCookieService {
    ...
    /**
     * Writes the {@code Set-Cookie} header with the shared attributes (HttpOnly, SameSite=None,
     * Secure). {@code SameSite=None} is required for the guest cookie to survive a cross-origin
     * request now that the frontend and backend are deployed as separate services/domains - which
     * in turn requires {@code Secure} unconditionally (browsers reject {@code SameSite=None}
     * without it). Modern browsers exempt {@code localhost} from needing real HTTPS for
     * {@code Secure} cookies, so this doesn't break local dev over plain http.
     */
